    ...
    const bool bottom_active)
{
    ...
    if (!bottom_active) if (dir_cosine[2]<0) return false;
    ...
    const double detector_lateral)
{
    ...
    if ((position[2]>detector_layer_position[2] && dir_cosine[2]<0) || (position[2]<detector_layer_position[2] && dir_cosine[2]>0))
        status = false;
    else {
